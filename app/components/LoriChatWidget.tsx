...
const defaultMessages: Message[] = [
  {
    id: 1,
    sender: "bot",
    text: "Hi there! 👋 I'm Lori, your Village Green of Bear Creek AI assistant. What can I help you with today?",
  },
];
...
export default function LoriChatWidget() {
  ...
  const handleSend = () => {
    ...
    if (input.toLowerCase().includes("location")) {
      reply = "Village Green of Bear Creek is located in Sherman, TX.";
                    ...
                    onClick={() =>
                      handleQuick(
                        ...
                        item.label === "Plans"
                          ? plansText
                          : item.label === "Pets"
                            ? "🐾 Village Green of Bear Creek is pet friendly — cats and dogs are welcome."
                            : item.label === "Parking"
                              ? "🚗 Resident Parking is available for residents."
                              : "📝 Apply online anytime through the Village Green of Bear Creek resident portal.",
                      )
                    }
